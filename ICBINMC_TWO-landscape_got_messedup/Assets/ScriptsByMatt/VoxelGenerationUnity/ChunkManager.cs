...
//			ch.Start (); 
	...
	{
		//TODO: make a new system...
		//noisePatch is not ready untill all of its neighbors are ready.
		// when noise patches finish creating calculating
		// they 'send up a request' that all of their neighbors also calculate
		NoiseCoord ncoo = noiseCoordContainingChunkCoord (coord);
		if (!blocks.noisePatchAtCoordIsReady(ncoo))
		{
			//add neighbor patches to noisepatch list?
			/*
			NoisePatch npatch = blocks.noisePatchAtNoiseCoord(ncoo);
			if (npatch != null) {
				List<NoiseCoord> unreadyNeighborPatches = npatch.neighborPatchesWhoAreNotDoneSettingUp();
				foreach(NoiseCoord nco in unreadyNeighborPatches) 
				{
					NoisePatch neighbor = makeNoisePatchIfNotExistsAtNoiseCoordAndGet(nco);
					setupThesePatches.Add(neighbor);
				}
			}
			*/ // WANT??
...
//					makeNoisePatchIfNotExistsAtNoiseCoord (nco);
				NoisePatch np = makeNoisePatchIfNotExistsAtNoiseCoordAndGet(nco);
...
//		drawDebugLinesForBlockAtWorldCoord (m_testBreakMakeBlockWorldPos);
	...
	public Coord highestSurfaceBlockCoordAt(Coord c) 
	{
		...
		NoisePatch npAtCoord = blocks.noisePatchAtWorldCoord(c);
		int y = npAtCoord.highestPointAtWorldCoord(c);
		return new Coord (c.x, y, c.z);
