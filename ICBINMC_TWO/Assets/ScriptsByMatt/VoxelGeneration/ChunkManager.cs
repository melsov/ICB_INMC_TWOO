...
public class ChunkManager : MonoBehaviour 
{
	...
	public const uint CHUNKLENGTH = 8;
...
//	private List<Coord> createTheseChunks = new List<Coord>();
	...
	private List<Chunk> destroyTheseChunks = new List<Chunk>();
	private List<Coord> dbugFunnyChunks = new List<Coord>();
...
//			throw new System.ArgumentException ("looking for a chunk that was not on the map..." + coord.toString());
		...
		AssertUtil.Assert(ch != null, "thought we just check for this (make ch sep thread)");
...
//			dbugFunnyChunks.Add(ch.chunkCoord);
		}
...
//			if (!ch.hasStarted) {
			if (!ch.hasStarted) 
			{
				if (!ch.IsDone) // eliminates missing chunks?
				{
					AssertUtil.Assert(!ch.IsDone, "we are surprised. we're building a chunk that's already done?");
					ch.Start();
...
//		return getVeryCloseAndInFrontRange();
		...
		Coord halfR = new Coord (7, 4, 7); // larger than wActiveRealm
...
//					m_wantActiveRealm = wantActiveRealm; 
				...
				addToCreateTheseChunksWithDistanceFromPlayer(5, cameraPizzaAngle);
				yield return new WaitForSeconds (.2f);
				addToCreateTheseChunksWithDistanceFromPlayer(6, cameraPizzaAngle);
...
//			if (!createTheseChunks.Contains(chh.chunkCoord))
	...
	private Coord getChunkCoordCloseToCameraView(bool wantChunkFromSet) 
	{
		...
		int radLimit = 5;
		for(; squareRadius < radLimit; ++squareRadius)
		{
...
//				else if (!TestRunner.RunGameOnlyNoisPatchesWithinWorldLimits)
				if (!TestRunner.RunGameOnlyNoisPatchesWithinWorldLimits)
				{
					if (shouldBeDestroyingChunksNow ()) 
					{
						...
						if (chunk != null && !chunkMap.isChunkCurrentlyBuilding(chunk)) 
						{
	...
	private Chunk getADoneCalculatingChunkWithChunkList(List<Chunk> chunkList)
	{
		foreach(Chunk chunk in chunkList) {
			if (chunk != null)
				if (chunk.Update()) //TODO: figure out what we need here...
					return chunk;
		}
		return null;
	}
...
//	}
	...
	// CONSIDER: replace much of this system with CoordRadarUtil functions?
...
//		}
	...
	{
		MPSimpleCollider simpleCollider;
		simpleCollider = playerTransform.GetComponent<MPSimpleCollider>();
		
		AssertUtil.Assert(simpleCollider != null, "no simple collider on player?");
		
		simpleCollider.setBlockProvider(this.blocks);
	}
...
//		DebugLinesUtil.drawDebugLinesForChunkRange(m_dontDestroyRealm);		
//		DebugLinesUtil.drawDebugCubesForChunksOnDestroyList(destroyTheseChunks);
//		DebugLinesUtil.drawDebugCubesForChunkCoordList(dbugFunnyChunks, Color.yellow, new Coord(-4, 0, 1));
