...
//#endif
	...
	private bool neighborsReady = false;
	
	public bool patchReady {
		get {
			return this.IsDone && this.generatedBlockAlready; // && this.neighborsReady; // FOR NOW!!
		} 
	}
...
//		generatedNoiseAlready = true;
	...
	#region checkNeighbors done
	
	public List<NoiseCoord> neighborPatchesWhoAreNotDoneSettingUp() 
	{
		List<NoiseCoord> result = new List<NoiseCoord>();
		
		foreach(NeighborDirection ndir in NeighborDirectionUtils.neighborDirections()) 
		{
			NoiseCoord nudge = NeighborDirectionUtils.nudgeCoordFromNeighborDirection(ndir);
			
			if (!this.m_chunkManager.blocks.noisePatchExistsAtNoiseCoord(this.coord + nudge))
			{
				result.Add(this.coord + nudge);
				continue;
			}
			
			NoisePatch npatch = this.m_chunkManager.blocks.noisePatches[this.coord + nudge];
			
			if (!npatch.hasStarted) 
			{
				result.Add(this.coord + nudge);
			}
		}
